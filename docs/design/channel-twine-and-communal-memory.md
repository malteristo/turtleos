...
> **Refined 2026-07-28 (proposed, sanction pending).** The governing quantity is **audience** cardinality, not member cardinality — the constraint was always about referent stability. A record read by *one* person may address them as "you" even in a multi-member space, which is what makes per-member notes possible; a record read by *several* must narrate in third person. A solo river and a per-member note then fall under the same rule for the same reason. See [per-member-periodic-notes.md](per-member-periodic-notes.md) §2. Items 1, 3 and 4 above are unaffected.
