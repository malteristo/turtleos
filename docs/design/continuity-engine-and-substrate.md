...
3. **"Let's just talk about X."** Narrowing the focus. Primary path is conversational — the practitioner just says it, or Turtle offers when the topic clearly matches something alive. A `!` command is a power-practitioner shortcut, not the front door.
...
```
*Invisible: conversational narrowing set `current.scope` to that thread; scoped self-feed pulled the tagged session notes. No command was typed. A power practitioner could reach the identical state with `!focus vocabulary-firewall` — the shortcut, not the front door.*
...
**Conversational-first (primary path).** The practitioner rarely knows the right scope upfront — conversation reveals it. So narrowing happens *in dialogue*: the practitioner just says "let's focus on X," or Turtle **offers** when the eddy clearly matches an active thread (confirm only, never silent set). `!focus` is a **power-practitioner shortcut**, not the front door — see §5.2a.
...
> **Correction, 2026-08-17 — `current.scope` does not exist and this document was the last place saying it did.** The design above puts scope on `current.yaml`, one value per practice root. Implementation moved it to **`state/scopes.yaml`, keyed by channel id**, for a reason the design missed: narrowing one conversation must not narrow the others. Everything in §5.2a, §7 and §8 that says "set `current.scope`" means "set this eddy's entry in `scopes.yaml`" — `!focus` has always called `set_scope(pd, channel_id, …)`.
>
> The root-level field was left behind at `null` and a unit test asserted it *stays* `null`, which pinned a vestige rather than removing it. It cost something measurable: on 2026-08-12 Turtle read `scope: null` in a live file as evidence that `current.yaml` had been designed as the practice's orientation document and never inhabited, and built a hypothesis on it. Field and test removed; this note stays because a design record should say where it was overtaken rather than quietly read as current.
...
| **Narrowing (v4)** | Conversational-first is the primary path; `!focus` is a documented power-practitioner shortcut, never required |
