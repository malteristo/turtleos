...
Operational rules when Kermit's turtleOS instance hosts sovereign practitioners (the hosted practitioner, friends via river keys).
...
| Infrastructure health checks | Surfacing the hosted practitioner's conversation content to Kermit without her action |
...
Hosted practice roots live on the Mini under `~/workshops/`. CouchDB / LiveSync are **retired** (2026-06-19) — do not provision `couchdb_database` or expect Obsidian sync. Operator harvest uses git + `sync_practice_root.sh`.
...
**Share to family:** Once `shared-river` ships, use **Share eddy** (space target) per [design-share-eddy.md](../chapters/design-share-eddy.md) — digest act in parent + shared eddy invitation; sharer not auto-joined; notify on first peer reply. Does not require duplicating share mechanics in this chapter.
