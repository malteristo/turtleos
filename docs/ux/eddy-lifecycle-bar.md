...
Practitioners who prefer typed commands keep turtle-talk (`!checkpoint`, `!release`, `!dissolve`). Buttons call the **same handlers** as commands.
...
6. **Seneschal act rows use the same River path** — Turtle suggests moment-specific acts in prose; River posts a temporary button row after the reply (native: excludes checkpoint / release / dissolve from seneschal rows; the bar itself renders flow pick · checkpoint · share, and release/dissolve are typed). Lifecycle bar content stays constant; one re-anchor per turn keeps it last.
...
| **`!` commands** | Eddy (River harness when split-bot) | Power-practitioner instant path; same handlers |
...
| Intake eddy, practitioner never reaches Begin | No lifecycle bar until dialogue is live |
