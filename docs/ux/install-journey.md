...
*Surface: Terminal. Practitioner may have an agent running these commands; copy below is what **they** should see.*
...
*Surface: Static checklist page (markdown or simple HTML). Practitioner keeps terminal open; works in browser tabs.*
...
|------|-----|-----------------|
| Your Discord ID | Right-click your name → Copy User ID | `DISCORD_USER_ID` |
...
### What the practitioner sees (sequence)

| Order | What appears | Practitioner interpretation |
|-------|----------------|---------------------|
| 1 | Thread titled **`new eddy`** | “Empty chat room” |
| 2 | Practitioner types: *“Hey — testing my install.”* | Normal Discord |
...
- Orientation essay before practitioner speaks in a new eddy  
...
Practitioner in Cursor/Claude Code:

> “Install turtleOS using the install skill.”

Agent runs `turtle install` → pauses at **Discord checklist** → practitioner pastes tokens → agent runs `turtle install connect` + verify.
...
## Failure & recovery (practitioner-facing)

| Symptom | Practitioner message | One action |
|---------|--------------|------------|
...
5. **Welcome embed timing** — Pin on first verify success, or only after practitioner's first successful J1 self-report?

6. **LaunchAgent default** — Opt-in “start on login” during connect, or separate `turtle install service` step?

7. **Naming** — Application names `River` / `Turtle` vs custom-branded names (confusing in member list)?
