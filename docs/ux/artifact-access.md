...
We do **not** pop up browsers unprompted except when you tap Open or use `!read`. Layer 1 practitioners who only chat may never open Artifacts. That is fine.
...
- Full practice-root tree in Discord (`!ls` everywhere) as the **product** experience — power-practitioner escape hatch only, narrowed over time.
