...
Operator provisions with `!admin invite <name> <emoji> [en|de] [--member @member]` (alias `river-key`).

- **Not yet on server:** send the channel invite from the reply.
...
```
!admin space create <practitioner-2>_play --members @admin @<practitioner-2> --policy all_practitioners
  → Discord #<practitioner-2>-play (members-only view by default)
  → registry spaces.<practitioner-2>_play + channels.* type shared-river
  → ~/workshops/<practitioner-2>_play/ seeded
...
Close when done: `!admin space close <practitioner-2>_play --confirm` — channel locked/archived; registry marked inactive.
