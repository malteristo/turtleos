...
Use this during a **phone/video walkthrough** (hosted river #2 — the second practitioner, 2026-06-26). Offline provisioning is not enough; verify both handoffs before ending the call.
...
1. Confirm guest state: **already on TurtleOS?** (common if you invited them to the server first) vs **not yet joined**.
2. Run `!admin invite <Name> <emoji> en` (alias `river-key`). If guest is already on the server, add `--member @them` (or username/id).
3. Handoff link depends on state:
   - **`--member` used / already on server** — send the **channel deep link** from the reply; door should already be open.
   - **Not yet on server** — send the **channel invite** from the command output (not a generic server invite). On join, turtleOS auto-admits them to the claim room when that invite is used. Fallback: `!admin rivers admit <name> @them`.
4. Optional preflight: `!admin members` should list humans (not only River). If bot-only → River Server Members Intent drift; fix with `!admin doctor` (see learnings 2026-07-25).
...
Claim rooms are created **operator-only** (`@everyone` cannot view). The guest is added to channel permissions only **after** claim (`_claimed_overwrites`) — unless you pre-grant.

The channel invite *should* grant visibility for a **new** join; on a fully private server it may not. If the guest **joined the guild earlier** (server invite before `invite`), the claim-room invite is especially unreliable: they are already a member, so Discord does not attach them to the private channel. Treat “guest is server member but not channel member” as expected until claim **or** operator pre-grant + deep link.

**Reference:** the second practitioner (2026-06-26, empty after join) · a third practitioner (2026-07-25, already on server before provision).
...
**Reference session:** the second practitioner (hosted river #2, 2026-06-26) — claim + eddy OK; empty-server + stale-registry fixed live.
