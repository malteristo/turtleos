...
| Principle | Meaning |
|-----------|---------|
| **Three tiers** | Tier 1: ignore account-local Discord prefs. Tier 2: structural registry sync. Tier 3: semantic lifecycle pipelines. |
...
**`dissolve_eddy(..., native_close=True)`** — thread may already be archived when the practitioner closed via Discord UI; skip early return and skip re-archive at end.
