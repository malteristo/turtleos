...
**Opened:** 2026-06-19  
**Closed:** 2026-06-19  
**Status:** Complete — dogfood with friend pending; the hosted practitioner grandfathered  
**First instance:** the hosted practitioner (`<practitioner-channel-id>`) on Kermit's server  
...
After the platform reorientation, the hosted practitioner's channel received v1 river chrome without onboarding. She asked *"Wer ist River?"* and *"Wer ist Eddy door?"* — legacy vocabulary and unexplained UX.
...
| **Trust rebuild** | the hosted practitioner's `resonance.md` is load-bearing — never push, never report to operator, demonstrate care through use. |
...
| `!admin onboard` workshop bootstrap | **Retired from help** — use `!admin invite` (`river-key` alias); see `design-admin-experience.md` |
...
```yaml
channels:
  '<practitioner-channel-id>':
    mage: <practitioner>
    type: hosted-river
    default_context: null
    description: The hosted practitioner's sovereign practice surface, hosted on Kermit's server
```
...
- [x] The hosted practitioner's character files + German onboarding (legacy path)
...
- [x] `!admin onboard` v1 alignment (later deprecated 2026-07-28)
- [x] TURTLE_SPEC §15 ripple

### Slice 2 — River keys / invite-to-claim (shipped)

- [x] `river_keys.py`, `!admin invite` / `river-key`, river_bot + discord_bot hooks
- [x] Claim room templates (de/en)
- [x] `#river-<name>` for life (no post-claim `*-dialogue` rename) — 2026-07-28
- [x] Tests
...
- [x] The hosted practitioner's eddy dogfood when she returns (operator — pending)
...
- `~/workshops/<practitioner>/resonance.md` on Mini
