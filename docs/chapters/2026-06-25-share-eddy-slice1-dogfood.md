...
**Share eddy (thinking together)** shipped as a single primitive with practitioner routing. Kermit dogfooded multiple shares to the hosted practitioner (birthday heat logistics, fungal networks, install test eddy). The core loop works: synthesize → preview → confirm → recipient `@` + digest + Continue → received eddy with seeded history.
...
| S1 — Recipient Continue + first-reply notify | **Pending** — awaits practitioner-side completion |
