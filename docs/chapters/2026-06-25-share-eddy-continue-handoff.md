...
When the hosted practitioner taps **Continue** on a share digest in her river:
...
2. Kermit: fresh eddy → `!share` → the hosted practitioner
3. The hosted practitioner: tap **Continue** once on one share
4. **Pass criteria (v1):**
   - **No River success ephemeral** on Continue
   - System thread-start line + chip in river (or sidebar thread visible)
   - **Digest embed inside eddy**; the hosted practitioner can talk; Turtle has context
5. Optional: thread interior screenshot (mobile DE); disk checks under `~/workshops/<practitioner>/share/`
