...
**First dogfood:** `<practitioner-2>_play` sandbox (S5 substitute — guest sharer outside channel)
...
```bash
# Kermit + the second practitioner sandbox; the hosted practitioner can share in without channel membership
!admin space create <practitioner-2>_play --members @kermit @<practitioner-2> --policy all_practitioners

# Family-like closed space
!admin space create family_trip --members @kermit @<practitioner> --policy members_only --context family

# Archive when done
!admin space close <practitioner-2>_play --confirm
```
...
```yaml
spaces:
  <practitioner-2>_play:
    practice_dir: ~/workshops/<practitioner-2>_play
    runtime_dir: ~/workshops/<practitioner-2>_play
    members: [kermit, <practitioner-2>]
    share_policy: all_practitioners

channels:
  "CHANNEL_ID":
    mage: <practitioner-2>_play
    type: shared-river
    default_context: shared
    description: Shared practice space (<practitioner-2>_play)
```
