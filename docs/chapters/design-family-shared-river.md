...
**Instance:** Family channel (`<family-channel-id>`)  
**Spec trace:** TURTLE_SPEC §15 (multi-practitioner / shared channels)

## Tension

The family channel was registered as `type: shared` with `default_context: family`. The Mage and his partner use it for **shared practice** — coordination, parenting, connection — with eventual participation from the children.
...
| **Privacy firewall** | No content from either partner's private river crosses into family — load-bearing |
...
spaces:
  family:
    ...
    members: [kermit, partner]   # children: future
...
| Phase | Action |
|-------|--------|
| **Design session** | Both partners: family compass, what shared practice means now, the children's boundaries |
...
5. Explicit privacy reaffirmation for the partner
