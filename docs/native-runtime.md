...
Turtle's local-only dialogue path now treats thread state as a thread card, not a thin log. In Discord threads, each meaningful exchange rewrites a markdown card under the runtime thread-state directory with the last practitioner move, last Turtle move, continuity cue, and return rule. On the next reply inside that thread, the card is injected into the runtime environment so Turtle can return to written state before simulating memory.
