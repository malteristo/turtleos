...
2. SSH to the Mac Mini (turtle@<turtle-host>):
...
```bash
ssh turtle@<turtle-host> 'cd ~/turtleos && git pull && ./scripts/shake_after_deploy.sh'
SHAKE_LIVE=1 ssh turtle@<turtle-host> 'cd ~/turtleos && SHAKE_LIVE=1 ./scripts/shake_after_deploy.sh'
```
