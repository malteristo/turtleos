...
Spirit harvests reports from `desk/craft/automation-reports/latest.md` at `. craft` on Forge — a duty in the Magic arrival flow's side-effect list since 2026-08-06. **It was not one before.** This line claimed the reader for six weeks while nothing in the flow called for it; 45 reports accumulated, 10 of them FAIL, and no session note or briefing ever cited one. Naming a reader in a registry does not make the reader read — the same defect §11 of `docs/development.md` catalogues for artifacts, here applied to a procedure.
