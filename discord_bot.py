...
Modular architecture (2026-03-29 refactor):
  ...
  background.py — background tasks (reminders, canary, daily notes)
        ...
        try:
            ...
            for t in archived_threads:
                ...
                if is_dissolved(t.id):
                    ...
                    continue
                # A cooled eddy is archived on purpose. This loop checked
                # `is_dissolved` and not `is_eddy_cooled`, so every boot undid the
                # retirement of anything cooled rather than dissolved — which is
                # the whole of "eddies age, don't close" (2026-07-20), reversed
                # once per restart, silently, by the code whose job is to rejoin
                # live threads. Found 2026-08-17 while checking whether a cool
                # survives a deploy.
                if is_eddy_cooled(t.id):
                    print(f"Skipped cooled archived thread: {t.name} (id: {t.id})")
                    continue
                try:
...
async def on_member_join(member):
    ...
    admitted = None
    try:
        admitted = await try_auto_admit_on_member_join(member)
    except Exception as exc:
