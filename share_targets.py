...
def river_channel_for_mage(mage_key: str) -> int | None:
    ...
    Archived rivers are not addressable: a retired river is one the member can
    no longer see, so offering it as a share target would deliver into silence.
...
def list_practitioner_targets(
    ...
    """Picker: registered mages with a river the sender may reach.

    Reach is relation-governed (``mage.may_reach``) — members address their own
    circles, an administrator addresses everyone, and a guest's only route is
    to their host. See `docs/design/relations-and-membership.md`.
    """
    sender_id = str(sender_discord_id)
    registry = get_registry()
    targets: list[ShareTarget] = []
    for mage_key, mage in registry.get("mages", {}).items():
        if mage_key == sender_mage_key:
            continue
        discord_id = mage.get("discord_id")
        if not discord_id or str(discord_id) == sender_id:
            continue
        if not may_reach(sender_mage_key, mage_key, registry):
            continue
...
def space_member_discord_ids(
    ...
    *,
    ...
    """Discord account ids for space members (optional exclude — e.g. sharer)."""
...
def mage_key_for_discord_id(discord_id: str | int) -> str | None:
    """Registry mage key for a Discord account id, if registered."""
    aid = str(discord_id)
