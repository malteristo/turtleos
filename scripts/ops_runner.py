...
def _suite_steps() -> list[tuple[str, list[str]]]:
    """Unit gate, then every shake in the one suite list.

    This used to be a second hand-maintained copy of the shake list. It drifted:
    `shake_report.DEFAULT_OFFLINE_SUITE` looked authoritative, was emitted into
    the report, and ran nothing — so six shake scripts went unrun for weeks and
    editing the visible list would not have fixed it.
    """
    from scripts.shake_report import offline_suite_steps

    return [("unittest", ["-m", "unittest", "discover", "-s", "tests", "-q"]), *offline_suite_steps()]


SUITE_STEPS: list[tuple[str, list[str]]] = _suite_steps()
...
def _update_summary(check: dict[str, Any]) -> str:
    ...
    # The caveat belongs on the line a human actually reads, not only in the
    # JSON block below it.
    source = check.get("source", {}) or {}
    if source.get("tracking_ref_stale"):
        remote_head = str(source.get("remote_head_sha") or "")
        suffix = f" (remote {remote_head[:8]})" if remote_head else ""
        parts.append(f"tracking ref STALE{suffix}")
    if dirty:
        parts.append("dirty")
    return ", ".join(parts)


WRITE_PATH_WINDOW_DAYS = 30


def _collect_write_paths() -> dict[str, Any]:
    """Per-offer take rates across every registered root."""
    try:
        from mage import list_registered_practice_dirs
        from offer_ledger import (
            KINDS,
            default_window_start,
            opened_on,
            render_write_path_section,
            tally,
        )

        roots = list_registered_practice_dirs()
        counts = tally(roots, since=default_window_start(WRITE_PATH_WINDOW_DAYS))
        opened = opened_on(roots)
        return {
            "window_days": WRITE_PATH_WINDOW_DAYS,
            "roots": len(roots),
            "opened": opened.isoformat() if opened else None,
            "counts": counts,
            "section": render_write_path_section(
                counts,
                window_days=WRITE_PATH_WINDOW_DAYS,
                known_kinds=KINDS,
                opened=opened,
...
def _collect_record_gaps() -> dict[str, Any]:
    ...
    try:
        from mage import list_registered_practice_dirs
        from core.record_gaps import (
            KINDS,
            default_window_start,
            render_record_gaps_section,
            tally,
        )
        ...
        return {
            ...
            "roots": len(roots),
            "counts": counts,
            "section": render_record_gaps_section(
                counts, window_days=WRITE_PATH_WINDOW_DAYS, known_kinds=KINDS
            ),
        }
    except Exception as exc:  # pragma: no cover - reporting must not break the gate
        return {"error": f"{type(exc).__name__}: {exc}"}
