...
# ─── Craft eddy write path ────────────────────────────────────────────
#
# Spirit cannot type in Discord. Everything the readiness seam needs — a
# proposal reaching a stranded eddy, a refusal's gap reaching the thread — has to
# be posted by something holding a live client, and the only two candidates were
# a command the Mage types (which is the marking act that has failed twice: the
# moves channel, the handoff flag) and this server, which already runs inside the
# bot process with `app["discord_client"]`.
#
# So it is here, and it is local-only by the same rule as `/shell`: reachable
# over SSH, unreachable from off-host without a token. No new secret.
#
# Posting in-process is also what makes the button work. A `View` attached to a
# message the running client sends is registered in that client's view store, so
# the confirm answers immediately; `rehydrate_ready_views` covers the same view
# after a restart, from the sidecar's `proposed` rows. A component posted over
# REST from outside would render a button that does nothing, which is this
# codebase's most-logged defect shape.
#
# Deliberately absent: the **spark** — a one-line re-entry hint for a cold eddy.
# The mechanism would be identical and the design is not settled (what it says,
# where it lands, how scarce it has to be to not become the clutter it treats),
# so it is not built rather than built badly. Backlog:
# `2026-08-17-cold-eddy-spark`.


async def _resolve_craft_thread(request, thread_id: int):
    """The thread, if it is a craft eddy this endpoint may write to.
    ...
    if client is None:
        raise web.HTTPServiceUnavailable(text="no discord client in this process")
...
def _thread_id_from(data: dict) -> int:
    try:
        return int(str(data.get("thread_id", "")).strip())
    except (TypeError, ValueError):
        raise web.HTTPBadRequest(text="thread_id required") from None


async def handle_craft_ready(request):
    """Propose readiness on an eddy and post the confirm. Spirit's hand, his press."""
    if not _shell_endpoint_authorized(request):
    ...
    thread_id = _thread_id_from(data)
    thread = await _resolve_craft_thread(request, thread_id)

    from core.craft_readiness import ReadinessError, propose
    ...
    try:
        entry = propose(
            get_runtime_dir(),
            thread_id,
            target_condition=data.get("target_condition") or "",
            evidence=data.get("evidence") or "",
            by=data.get("by") or "spirit",
        )
    except ReadinessError as exc:
        # The gate said no. That is an answer, not a failure of the endpoint.
        return web.json_response({"ok": False, "error": str(exc)}, status=422)

    posted = await offer_ready_confirm(
        thread,
        entry["target_condition"],
        entry.get("evidence", ""),
        runtime_dir=get_runtime_dir(),
    )
    return web.json_response({"ok": True, "posted": posted, "state": entry["state"]})


async def handle_craft_gap(request):
    """Refuse readiness and put the gap where he will actually see it.

    Until this existed the gap was written to the sidecar and rendered on the
    craft board, and three files claimed it reached the Hearth. It did not.
    """
    if not _shell_endpoint_authorized(request):
        return web.json_response({"ok": False, "error": "not authorized"}, status=403)
    try:
    ...
    from core.craft_readiness import ReadinessError, mark_gap_posted, refuse
    from mage import get_runtime_dir
    ...
    await thread.send(
        "**Not ready for the forge yet** — one thing is missing:\n\n"
        f"> {entry['gap']}\n\n"
        "That is the next thing to talk about here."
    )
    # Only after the send. A refusal recorded as posted when the post failed is
    # the same lie in the other direction.
    entry = mark_gap_posted(get_runtime_dir(), thread_id)
    return web.json_response({"ok": True, "state": entry["state"], "posted": True})


async def handle_craft_revise(request):
    """Sharpen or replace a confirmed target. `ready` is a waypoint.

    Posts the change into the eddy, because a target that moves silently is the
    defect this endpoint exists to fix, one level up: the practitioner confirmed
    a sentence and is entitled to see it when it stops being that sentence.
    """
    if not _shell_endpoint_authorized(request):
        return web.json_response({"ok": False, "error": "not authorized"}, status=403)
    try:
    ...
    from core.craft_readiness import REFINE, ReadinessError, revise
    from mage import get_runtime_dir

    kind = (data.get("kind") or REFINE).strip()
    try:
        entry = revise(
            get_runtime_dir(),
            thread_id,
    ...
    lead = "**Target sharpened**" if kind == REFINE else "**Target replaced** — this went somewhere else"
    await thread.send(f"{lead}:\n\n> {entry['target_condition']}")
    return web.json_response(
        {"ok": True, "kind": kind, "revisions": len(entry.get("target_history") or [])}
    )


async def handle_craft_spark(request):
    """Post the delta between a cooled eddy and a buildable target.

    The copy is the design. A gap posted after a proposal is a verdict and reads
    like one; a spark lands in a conversation he never submitted for judgement,
    so it has to read as an invitation. And it says out loud that the eddy is free
    to go somewhere else — the risk he named is that a pull toward a concrete
    target suffocates what might have emerged, and the cheapest guard against
    that is for the artifact itself to decline to insist.
    """
    if not _shell_endpoint_authorized(request):
        return web.json_response({"ok": False, "error": "not authorized"}, status=403)
    try:
    ...
    from core.craft_readiness import ReadinessError, mark_sparked
    from mage import get_runtime_dir
    from thread_registry import load_registry

    entry = (load_registry().get("threads") or {}).get(str(thread_id)) or {}
    try:
        row = mark_sparked(
            get_runtime_dir(),
            thread_id,
    ...
    await thread.send(
        "**Close to being a piece of work.** One thing would have to land first:\n\n"
        f"> {row['spark']}\n\n"
        "If that settles, this is ready for the forge. And if the conversation "
        "goes somewhere else instead, that is a good outcome too."
    )
    return web.json_response({"ok": True, "spark_count": row["spark_count"]})


async def handle_craft_cool(request):
    """Retire a craft eddy whose work is done. **Cool, not dissolve.**

    The practice decided this in July — *"eddies age, don't close"*: the content
    survives in the eddy, day and week notes, and the river's "Closed eddy"
    notifications were noise. For an eddy whose outcome is already recorded
    elsewhere (a ticked backlog entry, a harvested surface), a dissolve would
    spend an inference writing an essence of a conversation whose result is
    already written down, and post a lifecycle message about it.

    **Cool first, then archive, and the order is the whole safety argument.**
    `close_eddy_from_archive_transition` decides between cool and *full dissolve*
    by asking the audit log whether a human did it and counting messages — and an
    archive performed by the bot is not obviously "automatic" to that check. But
    the same handler returns early on `is_eddy_cooled`, so marking cooled *before*
    archiving makes the transition a no-op instead of a coin flip. Reversible
    either way: unarchiving a thread restores it.

    No new state. `harvest_status: cooled` already exists and already has 205 rows
    from every other channel; craft is the one place it had never fired.
    """
    if not _shell_endpoint_authorized(request):
        return web.json_response({"ok": False, "error": "not authorized"}, status=403)
    try:
    ...
    if not reason:
        # An eddy retired with no reason is indistinguishable from one that was
        # dropped, which is the accumulation this is meant to end.
        return web.json_response(
            {"ok": False, "error": "reason required — why is this eddy done?"}, status=422
        )

    from thread_registry import is_eddy_cooled, mark_cooled

    already = is_eddy_cooled(thread_id)
    if not already:
        mark_cooled(thread_id)

    archived = bool(getattr(thread, "archived", False))
    if not archived:
        try:
            await thread.edit(archived=True, reason=f"craft cool: {reason}"[:400])
        except Exception as exc:
            # Cooled but not archived is a coherent state — the registry is the
            # source of truth for the board, and the thread can be archived by
            # hand. Report it rather than pretending the whole act failed.
            return web.json_response(
                {"ok": True, "cooled": True, "archived": False, "error": str(exc)}
            )
    return web.json_response(
        {"ok": True, "cooled": True, "archived": True, "was_already_cooled": already}
    )
...
def create_intake_app(discord_client=None, vortex_thread_id=None) -> web.Application:
    ...
    app.router.add_post("/craft/ready", handle_craft_ready)
    app.router.add_post("/craft/gap", handle_craft_gap)
    app.router.add_post("/craft/cool", handle_craft_cool)
    app.router.add_post("/craft/revise", handle_craft_revise)
    app.router.add_post("/craft/spark", handle_craft_spark)
