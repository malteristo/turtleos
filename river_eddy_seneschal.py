...
def _offer(*, kind: str, command: str) -> ContextualOffer:
    """Build a contextual offer from the runtime registry.

    The label and the description are the runtime's — River renders them and owns
    only the ``!command`` string, which is how an act is accepted on *this*
    surface. Four sites used to spell the same two descriptions by hand.
    """
    from runtime.offers import accept_action, description_for

    action = accept_action(kind)
    return ContextualOffer(
        kind=kind,
        description=description_for(kind),
        actions=[(action.label, command)],
    )
...
def pick_contextual_offer(
    ...
    *,
    ...
    if url:
        return _offer(kind="save", command=f"!fetch {url}")
    if not checkpoint_offer_skip_reason(
        practitioner_text, history, min_exchanges=min_exchanges
    ):
        return _offer(kind="checkpoint", command="!checkpoint")
    return None
...
def _log_contextual_posted(channel, kind: str) -> None:
    ...
    # A print is not a record: log files rotate, and "how often did this offer
    # fire" is the only number that separates a feature nobody wanted from one
    # nobody was ever asked about. Persist it beside the chronicle.
    #
    # Resolution belongs to the ledger, not here — an offer in the family river
    # belongs to the family ledger, and an offer in an unregistered channel
    # (every test fixture) belongs in no ledger at all. This function used to
    # resolve by hand and got that wrong for eight days.
    try:
        from offer_ledger import record_for_channel

        record_for_channel(getattr(channel, "id", None), kind=kind, event="offered")
    except Exception:
        pass


def _record_offer_suppressed(channel, kind: str, reason: str) -> None:
    """A gate that only prints is the defect it was built to fix."""
    from offer_ledger import record_for_channel

    record_for_channel(
        getattr(channel, "id", None), kind=kind, event="suppressed", detail=reason
    )
...
async def maybe_offer_home_plan_after_turtle_reply(
    ...
    *,
    ...
    # Structure says "plan"; register says whether a plan is wanted. The
    # seneschal rule from AGENTS.md, finally on Turtle's side of the house:
    # stay observational in emotional, relational, grief/SOS and casual
    # contexts. Runs only after the cheap filter passed — 17 times in 8 weeks.
    from offer_register import offer_is_welcome

    if not await offer_is_welcome(practitioner_text, title=getattr(channel, "name", None)):
        _log_contextual_skip(channel, "home_plan", "care_register")
        _record_offer_suppressed(channel, "home_plan", "care_register")
        return False
...
async def maybe_offer_date_keep_after_turn(
    channel,
    *,
    practitioner_message,
) -> bool:
    """River: Keep-style confirm when the member named a date-worthy commitment."""
    from dates import (
        locale_for_practice_dir,
        offer_date_keep,
    ...
    commitment = parse_date_commitment(text, locale=locale)
    if commitment is None:
        return False

    offered = await offer_date_keep(
        practitioner_message,
        commitment,
        practice_dir=pd,
        locale=locale,
    ...
    if offered:
        _log_contextual_posted(channel, "date_keep")
    return offered
...
def _offer_from_turtle_intent(intent) -> ContextualOffer | None:
    ...
    if intent.action == "checkpoint":
        return _offer(kind="turtle_checkpoint", command="!checkpoint")
    if intent.action == "save" and intent.url:
        return _offer(kind="turtle_save", command=f"!fetch {intent.url}")
    return None
...
async def maybe_offer_turtle_intent_after_turn(
    ...
    *,
    ...
    # signal files in the operator's runtime and zero `turtle_checkpoint` lines
    # in the river log took a source read to explain, because the log had
    # nothing to say. A skipped offer is an event; log it like one.
    if not river_bot_enabled():
        _log_contextual_skip(channel, "turtle_intent", "river_bot_disabled")
        return False
    parent_id = getattr(channel, "parent_id", None)
    if not parent_id:
    ...
    if not intent:
        _log_contextual_skip(channel, "turtle_intent", "no_pending_offer")
        return False
...
def _assistant_replied_for_practitioner_turn(
    ...
    """True when shared dialogue history has assistant prose after this practitioner turn."""
    needle = (practitioner_text or "").strip()
    if not needle:
        return False
    last_user_idx: int | None = None
