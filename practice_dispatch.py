...
import state
from state import SPIRIT_BOT_ID, _processed_messages, get_channel_lock
...
async def dispatch_incoming_message(message: discord.Message) -> None:
    ...
    if message.author.bot:
        ...
        elif state.client.user in message.mentions and is_practice_channel(message):
    ...
    if (
        ...
        async with lock:
            if await try_river_key_claim(message, state.client):
                return
        return
    ...
    if (
        ...
        if is_awaiting_flow_intake(message.channel.id, message.channel.parent_id):
            ...
            if not practitioner_message_ends_intake_wait(
