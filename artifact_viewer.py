...
# `craft/` carries the turtleOS development record projected from the workshop
# backlog, plus the moves written back from craft-turtle eddies. It is
# operator-only for a concrete reason, not a cautious default: backlog item ids
# are slugs of the Mage's own reports and several contain a family member's
# full name. A hosted practitioner must never reach this shelf.
OPERATOR_ONLY_PREFIXES = ("proposals/", "craft/")
...
def is_artifact_readable(rel_path: str, *, mage_type: str | None = None) -> bool:
    ...
    if mage_type != "practitioner" and rel.startswith("proposals/") and rel.endswith(".md"):
        return True
    if mage_type != "practitioner" and rel.startswith("craft/") and rel.endswith(".md"):
        return True
...
def is_artifact_directory(rel_dir: str, *, mage_type: str | None = None) -> bool:
    ...
    if rel_dir == "":
        return True
    if mage_type != "practitioner" and rel_dir in ("proposals", "craft", "craft/moves"):
        return True
    return False
...
def list_shelves(*, mage_type: str | None = None) -> list[tuple[Shelf, int]]:
    ...
    for shelf in SHELF_DEFS:
        # Derive the hiding rule from OPERATOR_ONLY_PREFIXES rather than naming
        # a shelf here. The literal `proposals` check meant adding a second
        # operator-only shelf listed it to practitioners while every read of it
        # was correctly denied — a shelf they could see and not open.
        if mage_type == "practitioner" and f"{shelf.key}/" in OPERATOR_ONLY_PREFIXES:
            continue
...
def list_shelf_artifacts(shelf_key: str, *, mage_type: str | None = None) -> list[str]:
    ...
    elif key == "craft" and mage_type != "practitioner":
        # Same shape as proposals, plus moves/: the digest is the shelf's
        # front page; moves are the work that came back from craft-turtle.
        # Without this branch the Craft shelf listed with count 0 while
        # craft/development.md was directly readable — another "see and
        # not open" face, this time empty instead of denied.
        base = os.path.join(pd, "craft")
        if os.path.isdir(base):
            for name in sorted(os.listdir(base), reverse=True):
                if name.endswith(".md"):
                    paths.append(f"craft/{name}")
            moves = os.path.join(base, "moves")
            if os.path.isdir(moves):
                for name in sorted(os.listdir(moves), reverse=True):
                    if name.endswith(".md"):
                        paths.append(f"craft/moves/{name}")
    else:
        return []
