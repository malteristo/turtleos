...
def _stamp(created_at):
    """Local time, with the date whenever it is not today.

    ``created_at`` is UTC. Rendering it bare as %H:%M cost a forensic read on
    2026-07-29: a note posted 00:11 local showed as 22:11 and was placed on the
    wrong day, which inverted the sequence of two deploys. Local time fixes the
    value; the conditional date fixes the ambiguity that actually misleads,
    since a bare clock time silently crosses midnight.
    """
...
async def run():
    ...
    if op == 'read':
        ...
        for t in threads:
            ...
            created = t.created_at.astimezone().strftime('%Y-%m-%d') if t.created_at else '?'
