...
async def _archive_transition_is_deliberate(
    ...
    *,
    ...
    ``entry.changes`` is an ``AuditLogChanges``, which is **not iterable** — the
    old ``for change in entry.changes`` raised ``TypeError`` on every archive
    transition, the blanket ``except`` turned it into a log line, and the
    function returned False every time. So a practitioner deliberately closing
    an eddy was indistinguishable from Discord's auto-archive timer, and the
    full-dissolve branch below (including the ignore-purge path) has never once
    run: 10 archive transitions, 10 identical failures, 0 full dissolves against
    382 cool events in the live log. The pair to read is
    ``changes.before`` / ``changes.after``, each an ``AuditLogDiff`` whose
    ...
    try:
        ...
        async for entry in guild.audit_logs(limit=8, action=_discord.AuditLogAction.thread_update):
            if getattr(entry.target, "id", None) != after.id:
                continue
            if getattr(entry.changes.after, "archived", None) is True:
                return not getattr(entry.user, "bot", True)
    except Exception as exc:
        # Say what the failure *costs*, not just that it happened — this line
        # read as housekeeping for ten archives while it was the whole answer.
        print(
            f"Audit log fetch for archive transition failed ({after.id}): "
            f"{type(exc).__name__}: {exc} — treating the archive as automatic (cool, not dissolve)"
        )
    return False
