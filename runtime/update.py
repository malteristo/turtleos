...
def _divergence(
    *,
    ...
    # A stale tracking ref makes the local comparison meaningless: HEAD can
    # match `origin/main` exactly while the real remote is commits ahead.
    # Answering "up_to_date" from the stale ref is the report lying with a
    ...
    if stale:
        return {
            ...
            "behind": None,
            "reason": (
                f"local {base_ref} is stale; remote HEAD is "
                f"{remote_head[:8] or 'unknown'} — fetch before comparing"
            ),
        }
