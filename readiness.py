...
def assess_readiness(pd=None) -> dict:
    ...
    from background import interoception_loop, daily_reminders_loop, health_canary_loop
    ...
    if interoception_loop.is_running():
        ...
        metabolic_ok = False
        metabolic_detail.append("interoception ✗")
    # practice_health_loop retired 2026-08-02 — not a metabolic requirement
...
def _check_content_reach() -> dict:
    ...
    # YouTube: check tool (no auth needed). `_cli_path` verifies the shebang
    # resolves, so a relocated venv reads as degraded rather than as ready.
    ytdlp = _cli_path("yt-dlp")
    if ytdlp:
        tools["youtube"] = "ready"
    else:
        tools["youtube"] = "degraded"
        warnings.append("YouTube: yt-dlp not runnable (transcript API only)")
...
def check_content_reach_detail() -> str:
    ...
    if yt:
        ...
        lines.append("📺 **YouTube:** ⚠️ yt-dlp not runnable (using transcript API only)")
