...
API opt-in (optional):
  ...
  Craft-turtle eddies default to CRAFT_MODEL (frontier Claude) — the development
  surface is the intentional API opt-in, not the practice-river local stack.
...
def model_stack(use_api: bool) -> dict[str, ModelSlot]:
    ...
    return {
        ...
        "craft": {
            "model": CRAFT_MODEL,
            "backend": "API" if CRAFT_MODEL.startswith(("claude-", "gemini-")) else "local",
            "role": "Craft-turtle eddy dialogue",
        },
        "triage": {
            "model": TRIAGE_MODEL,
            "backend": "local",
