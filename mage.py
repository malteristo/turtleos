...
def get_effective_attunement(channel_id) -> str:
    """Effective attunement for a channel or eddy: override, craft type, or global.

    Resolves an eddy thread to its parent first. Without that, this returned the
    *global* profile for every thread — so `get_effective_attunement(eddy)` in
    `#craft-turtle` answered `native` while the parent answered `craft`, and the
    only reason most callers were right is that they remembered to pass
    `parent_id` themselves. Four did not (`share_delivery`, `eddy_spawn:1342`,
    `cmd_threads`, and `craft_intake` — the last is protected by its own explicit
    parent-only guard). Measured 2026-08-14; same shape as the offer ledger
    resolving a thread id against a parent-only registry the same morning.

    Callers that already pass a registered parent are unaffected:
    `resolve_registry_channel_id` is the identity on those.
    """
...
def _resolve_dialogue_channel_id() -> int | None:
    ...
    The docstring said so before it was true: this read the id out of `state`,
    which owns the client, so the whole `get_pd` chain below it imported the
    transport to look up a number. `CHANNELS` now lives in `core.config`.
    """
...
# ─── Relations (docs/design/relations-and-membership.md) ─────────

RELATIONS = frozenset({"household", "kin", "guest"})
DEFAULT_RELATION = "guest"

# Circles, not a ladder: who a member may reach across the node. Admin is an
# orthogonal capability and is handled outside this table — an admin reaches
# everyone, and everyone reaches an admin.
_REACH: dict[str, frozenset] = {
    "household": frozenset({"household", "kin"}),
    "kin": frozenset({"household", "kin"}),
    "guest": frozenset(),
}


def relation_for_mage(mage_key, registry: dict | None = None) -> str:
    """Relation class for a registry key. Unknown or absent → ``guest``.

    Fail-closed by design: a member nobody classified must never fall into the
    family by omission. Malformed values are reported by
    ``registry_relation_issues`` rather than honored.
    """
    registry = get_registry() if registry is None else registry
    mage = (registry.get("mages", {}) or {}).get(mage_key) or {}
    value = str(mage.get("relation") or DEFAULT_RELATION).strip().lower()
    return value if value in RELATIONS else DEFAULT_RELATION


def get_mage_relation() -> str:
    """Relation for the current channel's mage."""
    return relation_for_mage(_mage_key_ctx.get())


def is_admin_mage(mage_key, registry: dict | None = None) -> bool:
    """Whether a registry key holds administrator rights.

    ``admin: true`` is the field that means it. ``primary: true`` and
    ``default_mage`` keep granting it so existing single-operator registries
    are unchanged, but they mean "whose practice root is the default" — a
    second administrator needs ``admin``, not a second primary.
    """
    if not mage_key:
        return False
    registry = get_registry() if registry is None else registry
    ...
    if mage.get("admin"):
        return True
    return bool(mage.get("primary")) or mage_key == registry.get("default_mage")


def admin_discord_ids(registry: dict | None = None) -> set[int]:
    """Discord ids of every administrator. Canonical source for admin gates."""
    registry = get_registry() if registry is None else registry
    ids: set[int] = set()
    for key, mage in (registry.get("mages", {}) or {}).items():
        if not is_admin_mage(key, registry):
            continue
        raw = (mage or {}).get("discord_id")
        if raw is None:
            continue
        try:
            ids.add(int(raw))
...
def may_reach(sender_mage_key, target_mage_key, registry: dict | None = None) -> bool:
    """Whether one member may address another directly (share targets).

    Default deny across circles. Administrators reach every member and are
    reachable by every member — a guest's only route is to their host.
    """
    if not sender_mage_key or not target_mage_key:
        return False
    if sender_mage_key == target_mage_key:
        return False
    registry = get_registry() if registry is None else registry
    if is_admin_mage(sender_mage_key, registry) or is_admin_mage(target_mage_key, registry):
        return True
    sender = relation_for_mage(sender_mage_key, registry)
    target = relation_for_mage(target_mage_key, registry)
    return target in _REACH.get(sender, frozenset())


def registry_relation_issues(registry: dict | None = None) -> list[str]:
    """Admin-doctor lines for relation values the registry got wrong."""
    registry = get_registry() if registry is None else registry
    issues: list[str] = []
    for key, mage in (registry.get("mages", {}) or {}).items():
        raw = (mage or {}).get("relation")
        if raw is None:
            issues.append(f"`{key}` has no `relation` — treated as `{DEFAULT_RELATION}`")
            continue
        if str(raw).strip().lower() not in RELATIONS:
            issues.append(
                f"`{key}` has unknown relation `{raw}` — treated as `{DEFAULT_RELATION}`"
            )
    return issues
...
def _normalized_dir(path) -> str:
    """Expanded, symlink-resolved absolute path for registry comparison."""
    return os.path.realpath(os.path.expanduser(str(path)))


def registry_key_for_practice_dir(practice_dir):
    """Reverse-map a practice root to its registry key.

    Returns ``(key, kind)`` where kind is ``"mage"`` or ``"space"``;
    ``(None, None)`` when the root is not registered.
    """
    target = _normalized_dir(practice_dir)
    for section, kind in (("mages", "mage"), ("spaces", "space")):
        for key, entry in (_MAGE_REGISTRY.get(section) or {}).items():
            if not isinstance(entry, dict):
                continue
            configured = entry.get("practice_dir") or _default_runtime_dir_for_key(key)
            if _normalized_dir(configured) == target:
                return key, kind
    return None, None


def river_channel_id_for_mage_key(key: str) -> int | None:
    """The personal river channel belonging to a registry mage key."""
...
def address_for_mage_key(key: str) -> str:
    entry = (_MAGE_REGISTRY.get("mages") or {}).get(key) or {}
    return str(entry.get("address") or str(key).capitalize())


def river_channel_id_for_practice_dir(practice_dir) -> int | None:
    """The personal river channel that owns this practice root.

    Delivery of practitioner-facing artifacts (daily notes) must resolve from
    the root the artifact was synthesized for — never from the global dialogue
    channel, which publishes every root's material into the operator's river
    (INT-042).

    Returns None for shared-space roots: a space's periodic note is not one
    person's day, and routing it to any single river re-creates the §3.4
    asymmetry trap. Shared spaces route per-member once that lands — see
    ``docs/design/per-member-periodic-notes.md``.
    """
    if not (_MAGE_REGISTRY.get("channels") or {}):
        # Vanilla single-practitioner install: no registry, dialogue channel is it.
        return _resolve_dialogue_channel_id()

    key, kind = registry_key_for_practice_dir(practice_dir)
    if key is None or kind != "mage":
        return None
    return river_channel_id_for_mage_key(key)


def space_members_for_practice_dir(practice_dir) -> list[str]:
    """Member keys for a shared-space root; ``[]`` for a personal root.

    Story synthesis branches on this: one member keeps the intimate
    second-person voice, two or more take attributed witness voice
    (charter §3.3).
    """
    key, kind = registry_key_for_practice_dir(practice_dir)
    if kind != "space" or not key:
        return []
    space = _MAGE_REGISTRY.get("spaces", {}).get(key) or {}
...
def channel_is_shared_space(channel_id) -> bool:
    ...
    Charter §3.3 branches *voice* on member cardinality. Thread titles are
    durable, member-visible artifacts and branch on the same signal: above one
    member a title generated from one person's opening message would promote
    that person's framing of another into a permanent shared label, with no
    attribution attached to carry the "as she experienced it".
    """
    key = _get_channel_mage(channel_id)
    if not key:
        return False
    space = _MAGE_REGISTRY.get("spaces", {}).get(key) or {}
    return len(space.get("members") or []) > 1


def _discord_aliases(discord_id) -> set[str]:
    """Every name Discord may have written into stored history for this practitioner.

    Dialogue turns are stored under the *display name*, which the person owns,
    can change, and which differs per guild — while the practice thinks in the
    name the operator assigned at provisioning (usually a first name). Those
    are three separate name spaces reconciled only by ``discord_id``, the one
    stable identifier. Resolving live means the map never needs curating.

    Best-effort and cache-only: ``get_member`` is a local lookup (members
    intent is enabled), so this costs nothing and stays fresh.
    """
    aliases: set[str] = set()
    try:
        uid = int(discord_id)
...
def member_address_map() -> dict[str, str]:
    """Lowercased alias → practitioner address, for rendering shared records.

    A shared record should name people the way the practice names them, not by
    whatever handle Discord happened to store. Aliases come from Discord live
    (username, global name, display name, per-guild nick) and from the registry
    (key, address, optional ``display_name``); the registry wins, so the
    operator can always override.

    An unmapped handle passes through unchanged at the call site — a handle in
    the record is ugly but honest, and inventing a name would be worse.
    """
    out: dict[str, str] = {}
    for key, entry in (_MAGE_REGISTRY.get("mages") or {}).items():
        if not isinstance(entry, dict):
            continue
        address = str(entry.get("address") or key.capitalize())
        for alias in _discord_aliases(entry.get("discord_id")):
            out[alias.lower()] = address
        # Registry last: an explicit operator mapping beats a live lookup.
        out[str(key).lower()] = address
        out[address.lower()] = address
        display = entry.get("display_name")
        if display:
            out[str(display).lower()] = address
    return out
...
def list_registered_practice_dirs() -> list[str]:
    """Unique practice roots from registry — for root-explicit background jobs.

    INT-046: scheduled daily notes must iterate these rather than calling
    ``get_pd()`` with no context (which silently resolves to the primary).
    """
    seen: set[str] = set()
    out: list[str] = []
    ...
    for section in ("mages", "spaces"):
        for key, entry in (_MAGE_REGISTRY.get(section) or {}).items():
            if not isinstance(entry, dict):
                continue
            _add(entry.get("practice_dir") or _default_runtime_dir_for_key(key))
    return out


def current_practice_dir() -> str | None:
    """Practice dir from the active context only — never falls back to primary."""
...
def get_thread_member_ids(channel_id):
    ...
    For mage/practitioner channels: returns that practitioner's discord_id.
