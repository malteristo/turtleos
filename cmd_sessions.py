...
async def _announce_theme_harvest_if_any(message, result) -> None:
    """Announce what checkpoint already harvested — no Keep-these button."""
    note = getattr(result, "eddy_note", None)
    proposed = list(getattr(note, "proposed_themes", None) or [])
    if not proposed:
        return
    try:
        from continuity_confirm import announce_theme_harvest

        await announce_theme_harvest(message, proposed)
    except Exception as exc:
        print(f"Theme harvest announce failed: {type(exc).__name__}: {exc}")
...
async def cmd_checkpoint(message):
    ...
    await _announce_theme_harvest_if_any(message, result)
...
async def cmd_dissolve(message, args):
    ...
    Prepared eddies take a different path: ``!dissolve`` (optionally with a
    one-liner) writes ## Determination, sets disposition ready, and cools.
    Spirit harvests the determination into bright at `. craft` — not an LLM
    essence into thread-archive.
    """
    ...
    # Prepared branch — before the ordinary essence dissolve.
    try:
        from mage import get_pd, get_runtime_dir
        from core.prepared_eddies import (
            HARVESTED,
            READY,
            disposition_of,
            entry_for,
        ...
        prepared = entry_for(runtime, channel_id)
    except Exception as exc:
        prepared = None
        print(f"prepared-eddy check failed: {type(exc).__name__}: {exc}")

    if prepared is not None:
        disp = disposition_of(runtime, channel_id)
        if disp == HARVESTED:
            await message.reply(
                "This prepared eddy is already harvested into the practice.",
                mention_author=False,
            )
            return
        if disp == READY:
            await message.reply(
                "Already **ready for harvest** — Spirit folds the determination "
                "at the next `. craft`. The workspace holds it; no second dissolve needed.",
                mention_author=False,
            )
            return

        one_liner = " ".join(args).strip() if args else ""
        surface_rel = surface_of(runtime, channel_id)
        determination = one_liner
        if not determination and surface_rel:
            workspace = Path(get_pd()) / surface_rel
            if workspace.is_file():
                determination = read_determination(workspace.read_text(encoding="utf-8")) or ""

        if not determination.strip():
            await message.reply(
                "This is a **prepared eddy**. Ordinary dissolve would miss the "
                "determination.\n\n"
                "Write a `## Determination` section into the workspace, or run:\n"
                "`!dissolve <one sentence you can act on>`",
                mention_author=False,
            )
            return

        if not from_lifecycle_bar:
            await message.reply("Marking prepared eddy ready…", mention_author=False)
        try:
            from sessions import finish_prepared_eddy

            name, surface = await finish_prepared_eddy(
                channel_id,
                determination=determination,
                discord_client=discord_client,
                parent_channel_id=message.channel.parent_id,
                via_discord_ui=from_lifecycle_bar,
            )
        except ValueError as exc:
            await message.reply(f"Could not finish prepared eddy: {exc}", mention_author=False)
            return

        embed = discord.Embed(
            title="Prepared eddy ready",
            description=(
                f"**{name}** cooled with determination recorded.\n"
                f"Workspace: `{surface}`\n"
                "Spirit harvests into bright at `. craft` — check the artifact, not this message."
            ),
            color=0x2ECC71,
        )
