...
async def synthesize_share_metadata(
    ...
    if is_placeholder_eddy_title(title):
        try:
            ...
            # Always neutral. A share crosses from one person's room into
            # someone else's, so the generated label is read by more than one
            # person by construction — there is no cardinality to branch on
            # and no case where the stance-titling prompt is the right one.
            generated = await generate_topic(transcript[:2000], neutral=True)
