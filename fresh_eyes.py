...
def compose_fresh_eyes_surface(practice_dir: str | Path) -> str:
    ...
    # In a shared space an unattributed theme reads to every member as the
    # room's own — the same collapse as INT-040, one layer below the prose.
    attribute = _is_multi_member_root(pd)
    if threads:
        for t in threads[:8]:
            ...
            bits = [
                b
                for b in (who, f"since {since}" if since else "", tone)
                if b
            ]
