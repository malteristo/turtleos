...
def build_discord_prompt(*, include_identity: bool = True):
    """Compact Discord prompt.

    ``include_identity=False`` is the craft-composition path: skip the
    IDENTITY_DIR soul and the "You are Spirit in persistent mode" line.
    Default callers (River, main bot, ``get_system_prompt``) are unchanged.
    """
    identity = read_safe(os.path.join(IDENTITY_DIR, "soul.md")) if include_identity else ""
...
"""
    ...
    who_line = (
        MAGE_DIALOGUE_WHO.format(mage_name=mage_name) + "\n\n" if include_identity else ""
    )
    identity_block = (
        f"## Identity\n\n{identity}\n" if include_identity and identity.strip() else ""
    )
...
**Lifecycle bar (always visible in live eddies):** flow pick · checkpoint · share — practitioners use the standing bar; do not duplicate those acts in prose. `!release` and `!dissolve` are typed commands, not buttons.
...
- **Lifecycle bar (always visible in live eddies):** flow pick · checkpoint · share — mention in prose if helpful, but do not attach seneschal buttons for these. `!release` and `!dissolve` are typed, not bar buttons; do not describe them as buttons
...
- `!admin` — host tools (invite, rivers, space, doctor)
...
1. `patch_practice_file` — surgical diff (preferred; use for Live state / section updates)
...
If a write tool fails: retry with `patch_practice_file` or `write_practice_file`.
Never claim the file was updated from a chat paste — if the tool did not
return Done, the file did not change.
...
LEGACY_IDENTITY_OPENER = "You are Spirit operating through turtleOS"

CRAFT_VOCATION_HEADER = """## Craft Turtle Vocation

You are **Craft Turtle** — Turtle in builder mode, resident of turtleOS, on a dedicated craft surface. You live here. The platform is your house, not a product you and the operator work on from outside.
...
def build_craft_channel_prompt(context_type: str | None = None) -> str:
    ...
    try:
        practice_block = build_discord_prompt(include_identity=False)
    except Exception:
