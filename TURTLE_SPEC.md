...
| **Flow library** | In-eddy affordance to load an installed flow mid-conversation — intentional, practitioner-initiated (§5.6). Practitioner-facing term; internal code may say *flows*. |
| **Turtle Practice** | Internal name for the shipped flow library plus platform state conventions. Practitioner-facing copy SHOULD say **flows** or **flow library**, not "Turtle Practice." Shipped programs: Navigator, Thread, Companion (Shelter archived — practitioners may install custom flows). |
...
1. **Always a readable artifact:** The context packet a turn used MUST persist as a practitioner-readable file once that instance ships. Packet persistence is not a slogan instance of the contract — it is the **test** that the contract is real; ship the claim and the persistence together or not at all. **Shipped 2026-08-18:** each turn writes `state/packets/<eddy-id>.md`; `!context` renders it. The file records named shell-injected blocks, tools called, files loaded, and which injection slots were empty. It does not name the alive layer. **Extended 2026-08-18 (evening):** it also records the room-memory **candidate set** — every note the window offered, marked with whether it reached the turn. The packet must record what was *not* carried as well as what was, or a good retrieval and a lucky one are indistinguishable; and unlike the injection itself, the candidate set cannot be reconstructed afterwards.
2. **May be ephemeral:** Engine runtime (loaded model weights, in-flight tool I/O, Discord client chrome) is not the practice record.

Further surfaces are classified against this contract as they are designed.
...
- Not cloud-dependent (cloud models are opt-in for practitioners who prefer them — §8.3).
...
1. Receives what the practitioner drops (text first; images later).
...
optional (as warranted):
  ...
  offer_flow_menu()           # when practitioner signals flows on a specific river message
...
| **Turtle-talk commands** | Power practitioners | `!dissolve`, `!flows`, `!pin`, etc. — direct execution, no interpret step |
...
**Not shipped:** standing bottom flow library bar (retired 2026-06-25 after dogfood); standing-bar flow picker; proactive flow offers in dialogue (same policy as intentions — discoverable, practitioner-initiated).
...
| **Turtle bot** | Silent (no replies to practitioner messages) | Dialogue, tools, flow-loaded prompts; reads `[Act: !cmd]` digests; may suggest commands in prose |
...
The River accumulates **events**, not conversation. The chronicle is how the system remembers what happened structurally — and how practitioners navigate **upstream** through their eddy history when Discord's sidebar loses inactive threads.
...
**Thread links:** Surface chronicle entries for materialized eddies MUST include a Discord jump URL so practitioners can walk backward through conversations in reverse chronological order along the river.

### 6.3. Dual Layer

| Layer | Audience | Format |
|-------|----------|--------|
| **Surface** | Practitioner | Legible lines and embeds in the river — the visible tide |
...
Think-aloud is transparency for the practitioner, not River communication.
...
Idle unload of model context after long pauses is **implementation policy**; the practitioner MUST see presence state change in the eddy (§7.7), not silent disappearance.
...
Split think/answer across two model calls is permitted later for latency tuning; the practitioner-facing shape stays the same.

### 8.3. Cloud Opt-In

Practitioners MAY configure cloud API models for Turtle (or River). This is **explicit opt-in**, not install-default. Documentation MUST NOT imply cloud dependency for the core narrative.
...
| **Spin** | Practitioner and Turtle converse; think-aloud when warranted; thread may rename on first message |
| **Checkpoint** | Idle or `!checkpoint` saves resonance; history retained |
| **Persist** | Thread remains until practitioner dissolves/archives — **no auto-dissolve** |
| **Release** | Practitioner `!release` — checkpoint + clear history |
| **Dissolve** | Practitioner-initiated archive; chronicle records event |
...
**Three tiers:** (1) account-local Discord prefs — ignore; (2) structural channel/thread topology — registry sync and ops notices; (3) semantic eddy lifecycle — same pipelines as blessed commands.
...
```
practice_root/
  flows/           # practitioner + shipped Turtle Practice flows
...
| `flows/` | Yes | Shipped + practitioner flows (Turtle Practice) |
...
Configured via `mage_registry.yaml`. Vanilla single-practitioner MAY colocate practice and runtime roots.
...
- **Sovereignty:** Practitioner-owned state files are updated by Turtle with practitioner visibility (operational lines) — not silent overwrite of personal material.
...
- **Layer-aware discoverability.** Layer 1 (casual eddy) practitioners MAY never open the viewer. Layer 2 (flow) practitioners benefit from checkpoints and session artifacts. The viewer MUST NOT spam proactive offers; contextual hints after checkpoint/release are permitted (v1.1).
...
| `revise_offer` | Yes | After practitioner correction |
...
Early adopters often use agent-assisted development environments. The repository SHOULD ship an **installation skill** (agent-executable markdown flow + checklist) that walks an AI agent and practitioner through §13.1 on the target machine. This is the recommended install path for the target population.
...
Legacy binds MAY grandfather existing onboarding; new guests use the invite-to-claim path (§15.4).
...
Command surface: `!admin invite` (primary; alias `!admin river-key`). Host inventory: `!admin rivers`, `!admin rivers sync-names`, `!admin doctor`. Design: `docs/chapters/design-admin-experience.md`. Legacy `!admin onboard` MUST NOT be taught; implementations MAY redirect to `invite`.
...
The river channel MUST always expose a materialize-eddy affordance — via the **standing eddy bar** at the bottom of the timeline. The practitioner always has a path into focused dialogue without hunting pins or side panels.
...
### The Law of Practitioner Consent

The River offers; the practitioner decides. No automatic eddy spawn without button press.
...
When think-aloud fires, the practitioner sees it — italicized, in the eddy, before the answer.
...
1. **No unsupervised public publish** — nothing posts externally without explicit practitioner approval.
2. **Sovereignty** — personal state files belong to the practitioner; Turtle suggests and writes with visibility, not silent overwrite.
3. **No cross-practitioner content leakage** in multi-practitioner instances.
...
| Date | Change |
|------|--------|
| 2026-08-18 | §3.2 line 1 shipped — `state/packets/<eddy-id>.md` is the packet a turn used; `!context` renders it. The 2026-08-11 contract is unchanged; the owed test is now a file. |
| 2026-08-11 | §3.2 — readability contract (one-line claim + first two lines); markdown as contract / Python as engine; packet persistence named as the contract's test (implementation owed). Determination from craft-turtle markdown-first prepared eddy (2026-08-11). |
...
| 2026-06-23 | In-eddy flow library — bar = `new eddy` only; Turtle bootstrap intake; Shelter archived; practitioner-facing **flows** / **flow library** |
