...
async def load_thread_history(
    ...
    *,
    ...
    ``exclude_message_id`` omits the message currently being processed. The
    caller appends that turn itself after composing it (with attachment notes
    and injected context), so including it here stores it twice — INT-043.
    """
    ...
    try:
        async for msg in thread.history(limit=max_messages, oldest_first=True):
            if exclude_message_id is not None and msg.id == exclude_message_id:
                continue
            if msg.author.bot:
                if not (is_turtle_bot_message(msg) or msg.author == state.client.user):
                    continue
