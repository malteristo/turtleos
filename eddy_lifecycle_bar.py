"""In-thread lifecycle bar — River-owned flow pick / checkpoint / share affordances.

Release and dissolve are typed commands, not bar buttons; see ``LIFECYCLE_BAR_COMMANDS``
in ``cmd_dispatch`` for what the bar is treated as owning for seneschal purposes.
"""
...
async def _run_river_act_command(
    ...
    *,
    ...
    where = getattr(interaction.channel, "name", None) or getattr(interaction.channel, "id", "?")
    origin = "lifecycle bar" if from_lifecycle_bar else "act button"
    print(f"River act `!{cmd}` pressed on {origin} in #{where}")

    handler = DIRECT_COMMANDS.get(cmd)
    if not handler:
        print(f"River act `!{cmd}` unknown — no handler registered")
    ...
    print(f"River act `!{cmd}` completed on {origin} in #{where}")
...
class EddyLifecycleBarView(discord.ui.View):
    ...
    async def _on_flow_pick(self, interaction: discord.Interaction):
        ...
        # Rehydrated views carry no parent id (the bar state file does not hold one);
        # the thread the click arrived in does.
...
def rehydrate_lifecycle_bar_views(client) -> int:
    """Re-register a persistent view for every live lifecycle bar. Returns count.

    Without this a restart silently kills every posted bar: the buttons stay on
    screen and do nothing, because Discord routes the interaction to a client
    that no longer knows the custom_id. The practitioner sees an interaction
    failure with no log line on our side, which is the shape reported 2026-08-20.

    ``craft_ready_ui.rehydrate_ready_views`` learned this for one view class and
    the lesson was never carried to the others; this is the same repair for the
    bar. Views registered here carry no parent id — ``_on_flow_pick`` resolves it
    from the interaction instead.
    """
    count = 0
    for thread_id in list(_load_state()):
        try:
            phase = get_bar_phase(int(thread_id)) or "live"
            view = EddyLifecycleBarView(int(thread_id), 0, phase=phase)
            if not view.children:
                continue
            client.add_view(view)
            count += 1
        except Exception as exc:  # noqa: BLE001 — one bad row must not cost the rest
            print(f"Lifecycle bar rehydrate failed for {thread_id}: {type(exc).__name__}: {exc}")
    return count
