...
async def suggest_rename_title(
    thread_id: int, history_excerpt: str, parent_id: int | None = None
) -> str | None:
    """Best-effort title from thread content — optional opt-in rename only.

    ``parent_id`` selects neutral titling in shared spaces; a suggestion the
    member accepts becomes the same durable label as a generated one.
    """
    ...
    if len(seed) < 40:
        return None
    neutral = bool(parent_id) and channel_is_shared_space(parent_id)
    try:
        title = (await generate_topic(seed[:2000], neutral=neutral)).strip()[:100]
...
class FlowRenameOfferView(discord.ui.View):
    ...
    def __init__(self, thread_id: int, parent_id: int):
        ...
        rename_btn = discord.ui.Button(
            label=accept_action("flow_rename").label,
