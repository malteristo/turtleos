...
# River materializes every blank eddy under the literal name "new eddy" (and,
# pre-title, "new"). A *bare* name can therefore never identify an intake
# thread: the collision hijacks a practitioner's first message into an eddy
# spawn instead of a reply (INT-044). The only guard was is_native_river_eddy,
# which reads a marker that the title flow pops and that otherwise lives in the
# other bot process's memory — so it loses the race routinely.
# Emoji-prefixed forms ("🌀 new eddy") stay unambiguous and keep working.
BLANK_EDDY_NAMES = {"new", "new eddy"}
BARE_INTAKE_PATTERNS = INTAKE_PATTERNS - BLANK_EDDY_NAMES
...
# person's framing of another into a durable label the other person reads —
# and a title carries no attribution, so nothing marks it as one account.
# The reply rules say "she experiences it as"; a thread name cannot.
NEUTRAL_TOPIC_PROMPT = (
    "Generate a short, specific thread title (2-5 words) for a conversation "
    "about the following content. Several people share this space and every "
    "one of them will see this title, permanently.\n\n"
    "Title the SUBJECT, never the stance. If the content is one person's "
    "account of another, or a grievance, name the matter it concerns — not "
    "the accusation, not the characterization, not how anyone feels about "
    "anyone. Write \"the visit on monday\", not \"plans without me "
...
async def route_to_eddy(message, target_thread, url_content: str = ""):
    ...
    try:
        async with target_thread.typing():
            ...
            from tos_tools import execute_tos_tool, tools_for_channel
            reply, _ = await chat_ollama_with_tools(
                system_prompt, history[-20:],
                tos_tools=tools_for_channel(target_thread.id), execute_tool=execute_tos_tool,
            )
            if not reply:
                reply = "*listening*"
...
def is_intake_thread(channel) -> bool:
    ...
    if name_lower in BARE_INTAKE_PATTERNS:
        return True
...
async def generate_topic(content: str, *, neutral: bool = False) -> str:
    ...
    ``neutral`` titles the subject rather than the stance — required in spaces
    with more than one member, where the title is a shared durable artifact.
    """
    snippet = content[:1500]
    template = NEUTRAL_TOPIC_PROMPT if neutral else TOPIC_PROMPT
    try:
        result = await asyncio.wait_for(
            chat_ollama(
                "You generate short thread titles. Respond with ONLY the title, nothing else.",
                [{"role": "user", "content": template.format(content=snippet)}],
                model=TOPIC_MODEL,
                num_ctx=TOPIC_CTX,
                think=False,
            ),
...
async def spawn_eddy_in_channel(channel, content: str, topic: str | None = None,
    ...
    from mage import get_thread_member_ids, channel_is_shared_space
    from thread_registry import register_thread
    from state import client

    if not topic:
        topic = await generate_topic(
            content, neutral=channel_is_shared_space(channel.id)
        )
...
async def spawn_eddy(message, topic: str | None = None, eddy_type: str = "standard"):
    ...
    if not topic:
        topic = await generate_topic(
            text, neutral=channel_is_shared_space(message.channel.id)
        )
...
def resolve_turtle_bot_user_id(guild=None) -> int | None:
    """Turtle Discord account id for split-bot routing (env, cache, or guild lookup)."""
    import os
...
async def spawn_blank_river_eddy(
    ...
    *,
    ...
    if channel_att == "native":
        ...
        use_api = False
        model_label = "local"
    elif channel_att == "craft":
        from llm import resolve_model
        from state import CRAFT_MODEL

        model_id, use_api = resolve_model(CRAFT_MODEL)
        model_label = "api" if use_api else model_id
    else:
        from llm import resolve_model
    ...
    try:
        async with message.channel.typing():
            ...
            reply, _ = await chat_ollama_with_tools(
                ...
                tos_tools=tools_for_channel(thread_id), execute_tool=execute_tos_tool,
            )
            if not reply:
                reply = "*listening*"
    except Exception as e:
    ...
    try:
        async with thread.typing():
            ...
            reply, _ = await chat_ollama_with_tools(
                ...
                tos_tools=tools_for_channel(thread.id), execute_tool=execute_tos_tool,
            )
            if not reply:
                reply = "*listening*"
    except Exception as e:
