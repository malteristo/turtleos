...
async def rename_thread_from_flow_intake(
    ...
    title = (await generate_topic(seed, neutral=channel_is_shared_space(parent_id)))[:100]
...
def practitioner_message_ends_intake_wait(thread_id: int, parent_id: int) -> bool:
    """Talking is the answer to Prepare-or-talk. True when this cleared the wait.

    The Skip button this replaces said so in its own copy — *"your first message
    will bring Turtle in"* — while being the **only** exit from the wait. Eight
    call sites gate on `awaiting_intake`, and the first-message handler returned
    *before* clearing it, so an eddy whose practitioner simply started typing
    stayed frozen: no rename, no Turtle, no bar. Asking someone to announce what
    their next action already demonstrates is the button; this is the action.

    A flow with `intake.skippable: false` is unaffected. That is not an offer
    being ignored, it is a required step, and "ignoring is enough" is a rule
    about offers.
    """
    ...
    if not is_awaiting_flow_intake(thread_id, parent_id):
        return False
    data = read_awaiting_title(thread_id, parent_id) or {}
    spec = load_flow_spec(str(data.get("flow_id") or ""))
    if spec and spec.intake and spec.intake.skippable is False:
        return False
    patch_awaiting_title(
        thread_id, parent_id, awaiting_intake=False, intake_skipped=True
    )
    return True


class FlowIntakeOrientationView(discord.ui.View):
    """Prepare (modal) — shown on flow eddy materialize when intake is configured.

    There is no Skip. Sending a message is the skip; see
    `practitioner_message_ends_intake_wait`. Prepare stays available afterwards,
    so talking first costs nothing.
    """
    ...
    def __init__(self, thread_id: int, parent_id: int, flow_id: str):
        ...
        prepare_btn = discord.ui.Button(
            label=accept_action("flow_intake").label,
            style=discord.ButtonStyle.primary,
...
async def post_flow_intake_orientation(
    ...
    """Orientation embed with Prepare for flows that declare intake."""
    ...
    footer = (
        "Prepare recommended — or just start talking."
        if spec.intake.skippable
        else "Prepare to begin."
    )
    embed.set_footer(text=footer)
    view = FlowIntakeOrientationView(thread.id, parent_id, flow_id)
