...
def _write_alive(
    ...
    # Dated relative to today: threads decay out of the alive layer by age
    # (INFERRED_THREAD_TTL_DAYS), so a fixture pinned to a literal date stops
    # being "in motion" the moment the suite is run a week later.
    _today = _dt.date.today().isoformat()
    alive = {
        "version": 1,
        "updated_at": f"{_today}T08:00:00+02:00",
        "active_threads": [
            {"id": t, "label": t, "since": _today, "last_seen": _today,
             "tone": "tending"}
            for t in threads
        ],
...
class EddyNoteWriterTests(unittest.IsolatedAsyncioTestCase):
    ...
    def _patched(self, practice_dir: str, llm: AsyncMock) -> ExitStack:
        ...
        # Theme gate posts to Ollama and fails closed on timeout. Wiring it
        # into write_eddy_note turned two unit tests into live-inference tests
        # without anyone choosing that — green only when the slot is free.
        # Pass-through here; the gate itself is tested in test_theme_gate.py,
        # including an opt-in live path (TURTLEOS_LIVE_THEME_GATE=1).
        stack.enter_context(
            patch(
                "theme_gate.keep_topics",
                new=AsyncMock(
                    side_effect=lambda labels, **_kw: (
                        [l for l in (labels or []) if (l or "").strip()],
                        [],
                    )
                ),
            )
    ...
    async def test_write_eddy_note_runs_theme_gate_on_proposed_labels(self) -> None:
        """The unit suite mocks the gate; this pins that the call site still fires.

        Patching httpx would pass after the gate moved to chat_ollama_json.
        Patching keep_topics itself fails the moment write_eddy_note stops
        calling it — the same class of hole that left the orphan file inert.
        """
        llm = AsyncMock(
            return_value=_response(
                HELD_SLEEP,
                "none",
                None,
                proposed=["weekend trip packing"],
            )
        )
        gate = AsyncMock(return_value=(["weekend trip packing"], []))
        with tempfile.TemporaryDirectory() as tmp:
            with self._patched(tmp, llm) as stack:
                stack.enter_context(patch("theme_gate.keep_topics", new=gate))
                await story_notes.write_eddy_note(
                    CHANNEL_ID, HISTORY, trigger="idle"
                )
        gate.assert_awaited_once_with(["weekend trip packing"])
