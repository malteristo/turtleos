...
class TestEnsureChannelBars(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_craft_channel_schedules_reconcile(self) -> None:
        channel = MagicMock()
        channel.id = 99001
        channel.parent_id = None
        client = MagicMock()
    ...
    async def test_non_eddy_bar_parent_is_noop(self) -> None:
        channel = MagicMock()
        channel.id = 99
        channel.parent_id = None

        with patch("mage.supports_eddy_bar", return_value=False):
            with patch("bar_anchor.schedule_river_bar_reconcile") as schedule:
                await bar_anchor.ensure_channel_bars(channel)
