"""Tests for practice artifact allowlist (TURTLE_SPEC §11.5)."""

from __future__ import annotations

import os
import sys
import tempfile
import unittest
from unittest.mock import AsyncMock, MagicMock, patch, ANY

sys.modules.setdefault("discord", MagicMock())

import artifact_viewer as av


class TestArtifactAllowlist(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = tempfile.TemporaryDirectory()
        self.pd = self.tmp.name
        os.makedirs(os.path.join(self.pd, "sessions"), exist_ok=True)
        os.makedirs(os.path.join(self.pd, "proposals"), exist_ok=True)
        os.makedirs(os.path.join(self.pd, "state", "notes"), exist_ok=True)
        os.makedirs(os.path.join(self.pd, "thread-state"), exist_ok=True)
        open(os.path.join(self.pd, "sessions", "2026-06-29.md"), "w").close()
        open(os.path.join(self.pd, "proposals", "secret.md"), "w").close()
        open(os.path.join(self.pd, "state", "notes", "navigator.md"), "w").close()
        os.makedirs(os.path.join(self.pd, "story", "daily"), exist_ok=True)
        open(os.path.join(self.pd, "story", "daily", "2026-07-15.md"), "w").close()
        self.runtime = os.path.join(self.pd, "runtime")
        os.makedirs(os.path.join(self.runtime, "link-resonance"), exist_ok=True)

    def tearDown(self) -> None:
        self.tmp.cleanup()

    def test_practitioner_can_read_sessions(self) -> None:
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="practitioner"):
            self.assertTrue(av.is_artifact_readable("sessions/2026-06-29.md"))

    def test_practitioner_can_read_story_daily(self) -> None:
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="practitioner"):
            self.assertTrue(av.is_artifact_readable("story/daily/2026-07-15.md"))

    def test_practitioner_denied_proposals(self) -> None:
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="practitioner"):
            self.assertFalse(av.is_artifact_readable("proposals/secret.md"))

    def test_operator_can_read_proposals(self) -> None:
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="mage"):
            self.assertTrue(av.is_artifact_readable("proposals/secret.md"))

    def test_practitioner_denied_craft(self) -> None:
        """The craft shelf carries the turtleOS backlog projected from the
        workshop. Item ids are slugs of the Mage's own reports and several
        contain a family member's full name — a hosted practitioner reaching
        this shelf is a disclosure, not an inconvenience."""
        os.makedirs(os.path.join(self.pd, "craft", "moves"), exist_ok=True)
        open(os.path.join(self.pd, "craft", "development.md"), "w").close()
        open(os.path.join(self.pd, "craft", "moves", "2026-08-06-x.md"), "w").close()
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="practitioner"):
            self.assertFalse(av.is_artifact_readable("craft/development.md"))
            self.assertFalse(av.is_artifact_readable("craft/moves/2026-08-06-x.md"))
            self.assertFalse(av.is_artifact_directory("craft"))
            self.assertFalse(av.is_artifact_directory("craft/moves"))
            self.assertIsNone(av.resolve_artifact_path("craft/development.md"))
            keys = [s.key for s, _ in av.list_shelves(mage_type="practitioner")]
            self.assertNotIn("craft", keys)

    def test_operator_can_read_craft(self) -> None:
        os.makedirs(os.path.join(self.pd, "craft", "moves"), exist_ok=True)
        open(os.path.join(self.pd, "craft", "development.md"), "w").close()
        open(os.path.join(self.pd, "craft", "moves", "2026-08-06-x.md"), "w").close()
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="mage"):
            self.assertTrue(av.is_artifact_readable("craft/development.md"))
            self.assertTrue(av.is_artifact_readable("craft/moves/2026-08-06-x.md"))
            self.assertTrue(av.is_artifact_directory("craft/moves"))
            listed = av.list_shelf_artifacts("craft", mage_type="mage")
            self.assertEqual(
                listed,
                ["craft/development.md", "craft/moves/2026-08-06-x.md"],
            )
            keys = {s.key: n for s, n in av.list_shelves(mage_type="mage")}
            self.assertEqual(keys.get("craft"), 2)
            self.assertEqual(av.shelf_title_for_path("craft/development.md"), "Craft")

    def test_craft_read_tool_reaches_the_digest(self) -> None:
        """The gate that actually blocked this on first run was practice_io,
        not the viewer — they share an allowlist, and the tool is the path
        craft-turtle uses. Assert the tool, not just the predicate."""
        import practice_io

        os.makedirs(os.path.join(self.pd, "craft"), exist_ok=True)
        open(os.path.join(self.pd, "craft", "development.md"), "w").close()
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="mage"):
            self.assertTrue(practice_io.is_readable("craft/development.md"))

    def test_practitioner_can_read_story_eddy_notes(self) -> None:
        """Story surfaces are Tier-1 practitioner corpus (§11.5.1) — the
        checkpoint preview browser link (issue 036) resolves through here."""
        os.makedirs(os.path.join(self.pd, "story", "eddies"), exist_ok=True)
        open(os.path.join(self.pd, "story", "eddies", "2-morning-walk.md"), "w").close()
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="practitioner"):
            self.assertTrue(av.is_artifact_readable("story/eddies/2-morning-walk.md"))
            self.assertEqual(
                av.resolve_artifact_path("story/eddies/2-morning-walk.md"),
                os.path.join(self.pd, "story/eddies/2-morning-walk.md"),
            )

    def test_story_shelf_title(self) -> None:
        self.assertEqual(av.shelf_title_for_path("story/eddies/2-x.md"), "Story")

    def test_thread_state_denied(self) -> None:
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="practitioner"):
            self.assertFalse(av.is_artifact_readable("thread-state/card.md"))

    def test_list_shelves_hides_proposals_for_practitioner(self) -> None:
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="practitioner"):
            keys = [s.key for s, _ in av.list_shelves(mage_type="practitioner")]
            self.assertNotIn("proposals", keys)
            self.assertIn("sessions", keys)

    def test_list_shelf_artifacts_sessions(self) -> None:
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="practitioner"):
            paths = av.list_shelf_artifacts("sessions", mage_type="practitioner")
            self.assertEqual(paths, ["sessions/2026-06-29.md"])

    def test_discoverability_unlocks_on_corpus(self) -> None:
        with patch("artifact_viewer.get_pd", return_value=self.pd), patch(
            "artifact_viewer.get_runtime_dir", return_value=self.runtime
        ), patch("artifact_viewer.get_mage_type", return_value="practitioner"), patch(
            "artifact_viewer._discoverability_path",
            return_value=os.path.join(self.runtime, "artifact_discoverability.json"),
        ):
            self.assertTrue(av.artifacts_ui_eligible(mage_type="practitioner"))

    def test_checkpoint_hint(self) -> None:
        hint = av.checkpoint_artifact_hint(session_note="2026-06-29.md", flow_write=None)
        self.assertIn("!artifacts sessions", hint or "")


class TestSearchFormatting(unittest.TestCase):
    def test_format_includes_open_link_not_full_body(self) -> None:
        hits = [
            av.SearchHit("sessions/a.md", 1, "# Title"),
            av.SearchHit("sessions/a.md", 5, "some matching line here"),
        ]
        with patch("practice_io.PRACTICE_WEB_BASE", "http://127.0.0.1:8080"), patch(
            "practice_io.get_mage_key", return_value="kermit"
        ):
            text = av.format_search_results(hits, "match")
        self.assertIn("snippet", text.lower())
        self.assertIn("!read sessions/a.md", text)
        self.assertIn("http://127.0.0.1:8080/kermit/sessions/a.md", text)
        self.assertNotIn("# Title\n\nsome matching", text)


class TestCmdArtifacts(unittest.IsolatedAsyncioTestCase):
    async def test_menu_without_args(self) -> None:
        import cmd_practice_io as cpio

        message = MagicMock()
        message.channel.id = 12345
        message.reply = AsyncMock()
        fake_surface = MagicMock()
        fake_surface.embed = MagicMock(title="Recent")
        fake_surface.content = None
        fake_surface.open_actions = [("Open", "!read sessions/x.md")]
        with patch(
            "artifact_presenter.compose_artifact_surface", return_value=fake_surface
        ), patch("artifact_presenter.reply_artifact_surface", new_callable=AsyncMock) as reply:
            await cpio.cmd_artifacts(message, [])
        reply.assert_awaited_once()

    async def test_all_flag(self) -> None:
        import cmd_practice_io as cpio
        from artifact_presenter import ArtifactIntent

        message = MagicMock()
        with patch(
            "artifact_presenter.compose_artifact_surface", return_value=MagicMock(open_actions=[])
        ) as compose, patch("artifact_presenter.reply_artifact_surface", new_callable=AsyncMock):
            await cpio.cmd_artifacts(message, ["--all"])
        compose.assert_called_once_with(ArtifactIntent.BROWSE_ALL, mage_type=ANY)


if __name__ == "__main__":
    unittest.main()
