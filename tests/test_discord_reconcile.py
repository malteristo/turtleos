...
class TestHandleGuildChannelDelete(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_marks_orphan_and_logs(self) -> None:
        ...
        channel = MagicMock()
        channel.id = 555
        channel.name = "guest-play"
        registry = {
            "channels": {
                "555": {"type": "shared-river", "mage": "guest_play"},
            }
        }
...
class TestChannelBindingHint(unittest.TestCase):
    def test_practice_river_hint(self) -> None:
        ...
        self.assertIn("!admin invite", hint)

    def test_legacy_dialogue_hint(self) -> None:
        from discord_reconcile import _channel_binding_hint

        ch = MagicMock()
        ch.name = "guest-dialogue"
        ch.category = None
        hint = _channel_binding_hint(ch)
        self.assertIn("legacy", hint)
        self.assertIn("sync-names", hint)
...
class TestHandleGuildChannelCreate(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_posts_notice_for_unregistered(self) -> None:
        ...
        ch = self._text_channel(name="guest-play")
        ...
        log.assert_awaited_once()
        self.assertIn("guest-play", log.await_args.args[0])
