...
class SharedRiverHarnessTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_channel_is_river_uses_dialogue_env_without_discord_client(self) -> None:
        ...
        # Patched on `mage`, not on `state`: `CHANNELS` moved to `core.config`
        # on 2026-08-15 so that `get_pd` stopped reaching through the module
        # that owns the Discord client. The target of this patch is the
        # assertion — if it ever has to name `state` again, the layer moved.
        with patch("mage.CHANNELS", {"dialogue": str(self.RIVER_CHANNEL)}):
